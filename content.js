...
(function () {
  ...
  function addFinding(type, severity, message, el, confidence, evidence) {
    ...
    const normalizedText = safeText((evidence || message || "").toLowerCase(), 200);
    const existing = findings.find(f =>
      f.type === type && (f.el === el || safeText((f.evidence || f.message || "").toLowerCase(), 200) === normalizedText)
    );
    if (existing) return;
  ...
  const DECLINE_TEXT = /\b(cancel|decline|no thanks|reject all|opt out|unsubscribe|not interested)\b/i;
  // "skip" alone is too broad — it matches legitimate accessibility skip-links
  // ("Skip to content", "Skip navigation"), which are INTENTIONALLY hidden from
  // sighted users by design convention, not a dark pattern. Only match "skip"
  // when it's paired with commercial framing (skip an offer/deal/step), never bare.
  const SKIP_COMMERCIAL = /\bskip\s+(this\s+)?(offer|deal|step|promo|upgrade)\b/i;

  function isAccessibilitySkipLink(el) {
    const href = (el.getAttribute("href") || "").toLowerCase();
    const cls = (el.className && typeof el.className === "string" ? el.className : "").toLowerCase();
    const id = (el.id || "").toLowerCase();
    const text = textOf(el).toLowerCase();
    if (/^#(main|content|skip)/.test(href)) return true;
    if (/skip-?(link|nav|to-?content|to-?main)/.test(cls) || /skip-?(link|nav|to-?content|to-?main)/.test(id)) return true;
    if (/^skip (to )?(main )?(navigation|content)$/.test(text)) return true;
    return false;
  }
  ...
  function detectHiddenDecline() {
    document.querySelectorAll("button, a, [role='button']").forEach(el => {
      ...
      if (!text || isAccessibilitySkipLink(el)) return;
      const matchesDecline = DECLINE_TEXT.test(text) || SKIP_COMMERCIAL.test(text);
      if (!matchesDecline || !isVisible(el)) return;
  ...
  // Guard against false positives on articles/blog posts that merely DISCUSS
  // free trials and billing (e.g. "How SaaS Free Trials Work") rather than
  // actually being a live signup/checkout flow. We require a real, visible,
  // actionable control before treating this as a genuine risk signal.
  function hasActionableTrialControl() {
    const ctaPattern = /\b(start (your )?free trial|start (my )?trial|try (it )?free|start now|sign up now|subscribe now|activate (your )?trial|begin (your )?trial|get started free)\b/i;
    const controls = document.querySelectorAll("button, a[role='button'], input[type='submit'], a.btn, a.button, [class*='cta']");
    for (const el of controls) {
      if (isVisible(el) && ctaPattern.test(textOf(el))) return true;
    }
    return !!document.querySelector("form input[type='email'], form input[type='password'], form input[type='tel']");
  }
  ...
  function detectForcedContinuity() {
    ...
    const isLiveSignupFlow = hasActionableTrialControl();

    // Without a real, actionable signup control on the page, this reads as
    // editorial/informational content ABOUT trials, not an actual trap —
    // skip entirely rather than risk a false positive on a blog/article.
    if (!isLiveSignupFlow) return;
  ...
  function calculateSummary() {
    ...
    // Manipulation Score: 0-100, saturating curve so a handful of findings
    // already reads as clearly risky without needing dozens of detections.
    const manipulationScore = score === 0 ? 0 : Math.min(100, Math.round(100 * (1 - Math.exp(-score / 6))));
    return { score, high, medium, low, risk, manipulationScore };
  }

  // Search-engine results pages mostly display OTHER sites' text (snippets,
  // ads, cached descriptions) — scanning that text would misattribute a
  // third-party site's wording as this page's own manipulative UI. Skip
  // scanning entirely on known search-results URLs.
  const SEARCH_ENGINE_HOSTS = /(^|\.)google\.[a-z.]+$|(^|\.)bing\.com$|(^|\.)duckduckgo\.com$|(^|\.)search\.yahoo\.com$/i;
  function isSearchEngineResultsPage() {
    try {
      const host = location.hostname;
      if (!SEARCH_ENGINE_HOSTS.test(host)) return false;
      return /\/search/.test(location.pathname) || /[?&]q=/.test(location.search);
  ...
  function runAllDetectors() {
    ...
    if (isSearchEngineResultsPage()) {
      renderOverlay();
      try {
        chrome.runtime.sendMessage({ type: "DP_RESULTS", count: 0, summary: calculateSummary() }).catch(() => {});
      } catch (_) {}
      return;
    }
  ...
  function buildOverlay() {
    ...
    style.textContent = `
      ...
      .dp-score{margin:6px 14px 10px;font-size:13px;font-weight:800;color:#0f172a}.dp-score b{font-size:18px;color:#dc2626}
  ...
  function renderOverlay() {
    ...
    panel.innerHTML = `
      ...
      <div class="dp-score">Manipulation Score: <b>${summary.manipulationScore}</b>/100</div>
      ${items}
      <div class="dp-note">Heuristic signals are not legal conclusions. Review the evidence before deciding.</div>
      <button class="dp-btn" id="dp-download" style="margin:0 14px 12px;display:block">⬇ Download report</button>
    `;

    const dlBtn = shadowRoot.getElementById("dp-download");
  ...
  function downloadReport() {
    const summary = calculateSummary();
    const lines = [];
    lines.push("DARK PATTERN DETECTIVE — SCAN REPORT");
    lines.push("=====================================");
    lines.push(`URL: ${location.href}`);
    lines.push(`Scanned: ${new Date().toString()}`);
    lines.push(`Manipulation Score: ${summary.manipulationScore}/100 (${summary.risk})`);
    lines.push(`Findings: ${findings.length}  (High: ${summary.high}, Medium: ${summary.medium}, Low: ${summary.low})`);
    lines.push("");
    lines.push("This report lists heuristic signals only. These are NOT legal");
    lines.push("determinations. Review each item's evidence before relying on it.");
    lines.push("");
    findings.forEach((f, i) => {
      lines.push(`${i + 1}. [${f.severity.toUpperCase()}] ${f.type} (${f.confidence}% confidence)`);
      lines.push(`   ${f.message}`);
      lines.push(`   Evidence: ${f.evidence}`);
      if (f.ccpaCategory) lines.push(`   India CCPA 2023 category: ${f.ccpaCategory}`);
      lines.push("");
    });
    lines.push("Generated by Dark Pattern Detective — local-first, no data leaves the browser.");

    const blob = new Blob([lines.join("\n")], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    const safeHost = (location.hostname || "page").replace(/[^a-z0-9.-]/gi, "_");
    a.href = url;
    a.download = `dark-pattern-report-${safeHost}-${Date.now()}.txt`;
